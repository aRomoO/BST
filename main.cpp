...
int main() {
    ...
    assert(tree.whatLevelAmI(10) == 1); // Raíz
    assert(tree.whatLevelAmI(15) == 2); // Nivel 2
    assert(tree.whatLevelAmI(3) == 2);  // Nivel 2
    assert(tree.whatLevelAmI(18) == 3); // Nivel 3
    assert(tree.whatLevelAmI(7) == 3);  // Nivel 3
    ...
    tree.ancestors(7); // Debe imprimir "Ancestros de 7: 10 5 "
    std::cout << std::endl;
